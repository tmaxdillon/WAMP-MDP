...
output_path = ['~/MREL Dropbox/Trent Dillon/MATLAB/WAMP-MDP/' ...
    'output_data/theta_runs/r6_tp3_tA1000_tsl0/'];
    ...
    if slcomp %simple logic comparison
        ...
        for w = 1:size(mdpsim,1) %across all wcd
            for e = 1:size(mdpsim,2) %across all emx
                %disp(['w = ' num2str(w) ' e = ' num2str(e)])
                [t_r(e,w,4)] = ... 
                    calcThetaRate(mpnf_struct(w,e).output.a_act_sim, ...
                    mpnf_struct(w,e).output.FM_P(1,:,1), ...
                    mdpsim(w,e).mdp.tp)*100;
                [t_r(e,w,3)] = ...
                    calcThetaRate(apfl_3_struct(w,e).output.a_act_sim, ...
                    apfl_3_struct(w,e).output.FM_P(1,:,1), ...
                    mdpsim(w,e).mdp.tp)*100;
                [t_r(e,w,2)] = ...
                    calcThetaRate(apfl_4_struct(w,e).output.a_act_sim, ...
                    apfl_4_struct(w,e).output.FM_P(1,:,1), ...
                    mdpsim(w,e).mdp.tp)*100;
            end
        end
    end
