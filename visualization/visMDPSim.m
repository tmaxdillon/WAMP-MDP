...
hh = .99;
ll = .01;
[iav,~,~,~,~,~,imx] = calcIntermit(output.a_act_sim,99,1);
...
set(gca,'FontSize',fs)
grid on
title({['Avg P ' num2str(round(output.power_avg,2)) ...
    ', WEC = ' num2str(round(output.wec.rp,1)) ' W' ...
    ', Batt = ' num2str(round(output.wec.E_max/1000,1)) ...
    ' kWh, I_{mx} = ' num2str(imx) ' h,' ...
    ' I_{avg} = ' num2str(round(iav,3)) ' h,' ...
    ' J_r = ' num2str(round(mean(output.J_recon),2))],''})
xl = xlim;
xt = xticks;
%CWR TIME SERIES
ax(2) = subaxis(7,1,2);
...
for i = 1:max(output.a_sim)
    ...
    scatter(datetime(FM_P_1((output.a_act_sim==i),1),'ConvertFrom', ...
        'datenum'),output.a_act_sim(output.a_act_sim==i),'.', ...
