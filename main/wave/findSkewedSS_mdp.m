function [ss,y,Y,prob_max] = findSkewedSS_mdp(y,c,wec,Tpm)
...
for i = 1:length(y)
    [~,Y(i)] = skewedGaussian_mdp(y(i),c(1),c(2),1);
end
...
[~,res_ind] = min(abs(y - Tpm*wec.tp_res));
res_prob = Y(res_ind);
...
ss = (res_prob - prob_max)^2 + (med_prob - wec.med_prob*prob_max)^2;
